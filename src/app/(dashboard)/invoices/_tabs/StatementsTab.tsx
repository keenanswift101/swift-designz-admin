...
type Statement = {
  id: string; statement_number: string; period_type: string; period_from: string;
  period_to: string; trigger_type: string; opening_balance: number; total_invoiced: number;
  total_paid: number; closing_balance: number; sent_at: string | null; created_at: string;
  clients: { id: string; name: string; email: string | null; company: string | null } | null;
};

const TRIGGER_LABELS: Record<string, string> = {
  manual: "Manual", client_request: "Client Request", project_closure: "Project Closure",
  retainer_monthly: "Retainer Monthly", reminders_ignored: "Reminders Ignored", financial_year: "Financial Year",
};

export default async function StatementsTab() {
  const supabase = await createClient();

  const [{ data: statements }, { data: clients }, { data: projects }] = await Promise.all([
    supabase.from("account_statements")
      .select("id, statement_number, period_type, period_from, period_to, trigger_type, opening_balance, total_invoiced, total_paid, closing_balance, sent_at, created_at, clients(id, name, email, company)")
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 space-y-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="glass-card p-5"><p className="text-2xl font-bold text-foreground">{rows.length}</p><p className="text-xs text-gray-500 mt-1">Total Statements</p></div>
          <div className="glass-card p-5"><p className="text-2xl font-bold text-teal">{sentCount}</p><p className="text-xs text-gray-500 mt-1">Sent to Client</p></div>
          <div className="glass-card p-5"><p className="text-lg font-bold text-amber-400">{formatCurrency(totalOutstanding)}</p><p className="text-xs text-gray-500 mt-1">Total Outstanding</p></div>
        </div>
                  ...
                  {rows.map((s) => (
                    <tr key={s.id} className="hover:bg-card/50 transition-colors">
                      <td className="px-5 py-3">
                        <Link href={`/accounts-receivable/statements/${s.id}`} className="text-xs font-mono text-teal hover:underline">{s.statement_number}</Link>
                        {s.sent_at && <p className="text-xs text-gray-600 mt-0.5">Sent {formatDate(s.sent_at)}</p>}
                      </td>
                      ...
                      <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">{formatDate(s.period_from)} — {formatDate(s.period_to)}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right text-xs text-foreground tabular-nums">{formatCurrency(s.total_invoiced)}</td>
                      <td className="px-4 py-3 text-right text-xs font-semibold tabular-nums">
                        <span className={s.closing_balance > 0 ? "text-amber-400" : "text-teal"}>{formatCurrency(s.closing_balance)}</span>
                      </td>
