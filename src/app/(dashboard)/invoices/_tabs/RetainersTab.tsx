...
type RetainerPayment = { id: string; receipt_number: string; amount: number; payment_method: string; payment_date: string; sent_at: string | null };
type RetainerSub = {
  id: string; name: string; monthly_amount: number; billing_day: number;
  status: "active" | "paused" | "cancelled"; start_date: string; end_date: string | null;
...
export default async function RetainersTab() {
  ...
  const [{ data: retainers }, { data: clients }, { data: recentPayments }] = await Promise.all([
    ...
    supabase.from("retainer_payments")
  ...
  for (const p of (recentPayments ?? [])) {
    ...
    if (!paymentByRetainer.has(key)) paymentByRetainer.set(key, p as unknown as RetainerPayment);
    paymentCountByRetainer.set(key, (paymentCountByRetainer.get(key) ?? 0) + 1);
  }

  const rows = ((retainers ?? []) as unknown as Omit<RetainerSub, "payment_count">[]).map((r) => ({
    ...r,
  ...
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="glass-card p-5"><p className="text-2xl font-bold text-teal">{active.length}</p><p className="text-xs text-gray-500 mt-1">Active</p></div>
        <div className="glass-card p-5"><p className="text-2xl font-bold text-amber-400">{paused.length}</p><p className="text-xs text-gray-500 mt-1">Paused</p></div>
        <div className="glass-card p-5"><p className="text-2xl font-bold text-gray-400">{cancelled.length}</p><p className="text-xs text-gray-500 mt-1">Cancelled</p></div>
        <div className="glass-card p-5"><p className="text-lg font-bold text-foreground">{formatCurrency(monthlyRevenue)}</p><p className="text-xs text-gray-500 mt-1">Monthly Revenue</p></div>
      </div>
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-3 gap-6">
        <div className="2xl:col-span-2">
          {rows.length === 0 ? (
            ...
            <>
                  ...
                  <div key={r.id} className="glass-card p-4">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">{r.clients?.name ?? "—"}{r.clients?.company ? ` · ${r.clients.company}` : ""}</p>
                        <p className="text-xs text-gray-600 mt-0.5">Started {formatDate(r.start_date)}{r.end_date ? ` · ends ${formatDate(r.end_date)}` : ""}</p>
                      </div>
                      <span className={`shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border capitalize ${STATUS_STYLES[r.status]}`}>{r.status}</span>
                    </div>
                    <div className="flex items-center justify-between mb-3 border-t border-border pt-3">
                      <div><p className="text-xs text-gray-500">Billing day</p><p className="text-xs text-gray-400 mt-0.5">{billingDayLabel(r.billing_day)}</p></div>
                      <p className="text-base font-semibold text-foreground tabular-nums">{formatCurrency(r.monthly_amount)}<span className="text-xs text-gray-500 font-normal">/mo</span></p>
                    </div>
                    <div className="mb-3 text-xs">
                      {r.recent_payment ? (
                        <div className="flex items-center gap-1 text-gray-500">
                          <span>Last payment:</span>
                          <span className="text-foreground tabular-nums">{formatCurrency(r.recent_payment.amount)}</span>
                          <span>on {formatDate(r.recent_payment.payment_date)}</span>
                          {r.recent_payment.sent_at && <CheckCircle2 className="h-3 w-3 text-teal" />}
                        </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap border-t border-border pt-3">
                      <RecordRetainerPaymentButton retainerId={r.id} retainerName={r.name} clientName={r.clients?.name ?? "Client"} clientEmail={r.clients?.email ?? null} monthlyAmount={r.monthly_amount} paymentCount={r.payment_count} />
              ...
              {/* Desktop table */}
              <div className="hidden xl:block glass-card" style={{ overflow: "clip" }}>
                    ...
                    <tbody className="divide-y divide-border">
                      {rows.map((r) => (
                        <tr key={r.id} className="hover:bg-card/50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-xs text-gray-400">{billingDayLabel(r.billing_day)}</td>
                          <td className="px-4 py-3 text-right text-sm font-semibold text-foreground tabular-nums">{formatCurrency(r.monthly_amount)}<span className="text-xs text-gray-500 font-normal">/mo</span></td>
                          <td className="px-4 py-3">
                            {r.recent_payment ? (
                              <div>
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border capitalize ${STATUS_STYLES[r.status]}`}>{r.status}</span>
                          </td>
                          <td className="px-5 py-3">
        ...
        <div><NewRetainerForm clients={clients ?? []} /></div>
      </div>
    </>
  );
}
