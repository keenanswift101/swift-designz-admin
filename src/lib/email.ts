...
export async function sendQuotationEmail({
  ...
  const itemRows = lineItems.map((item) => `
    <tr>
      <td style="padding:10px 12px;font-size:13px;color:#ccc;border-bottom:1px solid #1a1a1a;">${escapeHtml(item.description)}</td>
      <td style="padding:10px 12px;font-size:13px;color:#888;text-align:center;border-bottom:1px solid #1a1a1a;">${item.quantity}</td>
      <td style="padding:10px 12px;font-size:13px;color:#888;text-align:right;border-bottom:1px solid #1a1a1a;">${fmtR(item.unit_rate)}</td>
      <td style="padding:10px 12px;font-size:13px;color:#fff;text-align:right;border-bottom:1px solid #1a1a1a;font-weight:600;">${fmtR(item.amount)}</td>
    </tr>`).join("");
    ...
    <tr>
      <td align="center">
        <table width="580" cellpadding="0" cellspacing="0" style="background:#111;border:1px solid #222;border-radius:12px;overflow:hidden;max-width:580px;width:100%;">
          ...
          <tr>
            <td style="padding:32px;">
              <p style="margin:0 0 6px;font-size:13px;color:#70c0c0;letter-spacing:1px;text-transform:uppercase;">Quotation for Approval</p>
              <h1 style="margin:0 0 24px;font-size:22px;font-weight:700;color:#fff;line-height:1.3;">${escapeHtml(quoteNumber)}</h1>

              <p style="margin:0 0 24px;font-size:14px;color:#aaa;line-height:1.6;">Hi ${escapeHtml(clientName)},</p>
              <p style="margin:0 0 24px;font-size:14px;color:#ccc;line-height:1.6;">
                ...
                This quotation expires on <strong style="color:#fff;">${expiryFormatted}</strong>.
              </p>

              <!-- Line items table -->
              <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #222;border-radius:8px;overflow:hidden;margin:0 0 24px;">
                <thead>
                  <tr style="background:#0a1a1a;">
                    <th style="padding:10px 12px;font-size:11px;color:#555;text-align:left;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Description</th>
                    <th style="padding:10px 12px;font-size:11px;color:#555;text-align:center;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Qty</th>
                    <th style="padding:10px 12px;font-size:11px;color:#555;text-align:right;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Rate</th>
                    <th style="padding:10px 12px;font-size:11px;color:#555;text-align:right;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Amount</th>
                  </tr>
                </thead>
                <tbody>${itemRows}</tbody>
                <tfoot>
                  <tr style="background:#0a1a1a;">
                    <td colspan="3" style="padding:12px 16px;font-size:14px;color:#aaa;font-weight:600;">Total</td>
                    <td style="padding:12px 16px;font-size:18px;color:#30B0B0;font-weight:700;text-align:right;">${fmtR(total)}</td>
                  </tr>
                </tfoot>
              </table>

              ${notes ? `<div style="background:#0d1a1a;border-left:3px solid #30B0B0;padding:12px 16px;margin:0 0 24px;border-radius:0 6px 6px 0;"><p style="margin:0;font-size:13px;color:#aaa;line-height:1.6;">${escapeHtml(notes)}</p></div>` : ""}
          ...
          <tr>
            <td style="padding:20px 32px;border-top:1px solid #1a1a1a;">
              <p style="margin:0;font-size:11px;color:#444;line-height:1.6;">Swift Designz &middot; swiftdesignz.co.za &middot; keenan@swiftdesignz.co.za</p>
            </td>
          </tr>
...
export async function sendInvoiceEmail({
  ...
  const scheduleRows = hasplan
    ? paymentPlanSchedule!.map((inst) => {
        ...
        const rowBg = isCurrent ? "#0a2020" : "transparent";
        const amtColor = isCurrent ? "#30B0B0" : "#aaa";
        const labelColor = isCurrent ? "#fff" : "#888";
        const instDate = new Date(inst.due_date).toLocaleDateString("en-ZA", { day: "numeric", month: "short", year: "numeric" });
        return `
    <tr style="background:${rowBg};border-bottom:1px solid #1a1a1a;">
      ...
      <td style="padding:9px 12px;font-size:12px;color:#666;text-align:center;">${instDate}</td>
  ...
  const planSection = hasplan ? `
    <!-- Payment plan -->
    <div style="background:#081818;border:1px solid #1a3030;border-radius:8px;margin:0 0 24px;overflow:hidden;">
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr style="background:#0a2020;">
          <td colspan="4" style="padding:10px 12px;font-size:11px;font-weight:700;color:#30B0B0;letter-spacing:2px;text-transform:uppercase;border-bottom:1px solid #1a3030;">
            Payment Plan — Instalment ${installmentNumber} of ${totalInstallments}
          </td>
        </tr>
        <tr style="background:#050e0e;">
          <th style="padding:8px 12px;font-size:10px;color:#444;text-align:left;font-weight:600;text-transform:uppercase;border-bottom:1px solid #1a1a1a;">#</th>
          <th style="padding:8px 12px;font-size:10px;color:#444;text-align:left;font-weight:600;text-transform:uppercase;border-bottom:1px solid #1a1a1a;">Description</th>
          <th style="padding:8px 12px;font-size:10px;color:#444;text-align:center;font-weight:600;text-transform:uppercase;border-bottom:1px solid #1a1a1a;">Due</th>
          <th style="padding:8px 12px;font-size:10px;color:#444;text-align:right;font-weight:600;text-transform:uppercase;border-bottom:1px solid #1a1a1a;">Amount</th>
        </tr>
        ${scheduleRows}
      </table>
      <p style="margin:0;padding:10px 12px;font-size:11px;color:#444;border-top:1px solid #1a1a1a;">
        The remaining instalments will be invoiced separately as they fall due.
      </p>
    </div>` : "";

  const introText = hasplan
    ? `Please find your invoice from Swift Designz attached as a PDF. This invoice covers <strong style="color:#fff;">Instalment ${installmentNumber} of ${totalInstallments}</strong> on your payment plan. Payment of <strong style="color:#30B0B0;">${fmtR(total)}</strong> is due by <strong style="color:#fff;">${dueDateFormatted}</strong>.`
    : `Please find your invoice from Swift Designz attached to this email as a PDF. Payment of <strong style="color:#30B0B0;">${fmtR(total)}</strong> is due by <strong style="color:#fff;">${dueDateFormatted}</strong>.`;
    ...
    <tr>
      <td align="center">
        <table width="580" cellpadding="0" cellspacing="0" style="background:#111;border:1px solid #222;border-radius:12px;overflow:hidden;max-width:580px;width:100%;">
          ...
          <tr>
            <td style="padding:32px;">
              <p style="margin:0 0 6px;font-size:13px;color:#70c0c0;letter-spacing:1px;text-transform:uppercase;">${hasplan ? `Tax Invoice — Instalment ${installmentNumber} of ${totalInstallments}` : "Tax Invoice"}</p>
              <h1 style="margin:0 0 24px;font-size:22px;font-weight:700;color:#fff;line-height:1.3;">${escapeHtml(invoiceNumber)}</h1>

              <p style="margin:0 0 16px;font-size:14px;color:#aaa;line-height:1.6;">Hi ${escapeHtml(clientName)},</p>
              <p style="margin:0 0 24px;font-size:14px;color:#ccc;line-height:1.6;">${introText}</p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #222;border-radius:8px;overflow:hidden;margin:0 0 24px;">
                ...
                <tfoot>
                  <tr style="background:#0a1a1a;">
                    <td colspan="3" style="padding:12px 16px;font-size:14px;color:#aaa;font-weight:600;">${hasplan ? `Instalment ${installmentNumber} Due` : "Total Due"}</td>
                    <td style="padding:12px 16px;font-size:18px;color:#30B0B0;font-weight:700;text-align:right;">${fmtR(total)}</td>
                  </tr>
                </tfoot>
              </table>
              ...
              <p style="margin:0;font-size:13px;color:#666;line-height:1.6;">
                Please use <strong style="color:#aaa;">${escapeHtml(invoiceNumber)}</strong> as your payment reference.<br/>
    ...
    <tr>
      <td align="center">
        <table width="520" cellpadding="0" cellspacing="0" style="background:#111;border:1px solid #222;border-radius:12px;overflow:hidden;max-width:520px;width:100%;">
          ...
          <tr>
            <td style="padding:32px;">
              <p style="margin:0 0 6px;font-size:13px;color:#70c0c0;letter-spacing:1px;text-transform:uppercase;">Payment Receipt</p>
              <h1 style="margin:0 0 4px;font-size:22px;font-weight:700;color:#fff;">${escapeHtml(receiptNumber)}</h1>
              <p style="margin:0 0 24px;font-size:12px;color:#555;">${dateFormatted}</p>

              <p style="margin:0 0 24px;font-size:14px;color:#aaa;line-height:1.6;">Hi ${escapeHtml(clientName)},<br/>Thank you — your payment has been received and confirmed.</p>

              <!-- Amount paid -->
              <table width="100%" cellpadding="0" cellspacing="0" style="background:#0a1a1a;border:1px solid #1a3030;border-radius:8px;margin:0 0 20px;">
                <tr>
                  <td style="padding:16px 20px;font-size:13px;color:#70c0c0;font-weight:600;">Amount Paid</td>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0a;border:1px solid #1a1a1a;border-radius:8px;margin:0 0 24px;">
                <tr><td colspan="2" style="padding:12px 16px;font-size:11px;font-weight:700;color:#30B0B0;letter-spacing:2px;text-transform:uppercase;border-bottom:1px solid #1a1a1a;">Details</td></tr>
                <tr>
                  <td style="padding:10px 16px;font-size:12px;color:#666;">Method</td>
                  <td style="padding:10px 16px;font-size:12px;color:#ccc;font-weight:600;text-align:right;text-transform:capitalize;">${escapeHtml(paymentMethod)}</td>
                </tr>
                <tr style="border-top:1px solid #111;">
                  <td style="padding:10px 16px;font-size:12px;color:#666;">Invoice</td>
                  <td style="padding:10px 16px;font-size:12px;color:#ccc;font-weight:600;text-align:right;">${escapeHtml(invoiceNumber)}</td>
                </tr>
                <tr style="border-top:1px solid #111;">
                  <td style="padding:10px 16px;font-size:12px;color:#666;">Invoice Total</td>
                  <td style="padding:10px 16px;font-size:12px;color:#ccc;font-weight:600;text-align:right;">${fmtR(invoiceTotal)}</td>
                </tr>
                <tr style="border-top:1px solid #1a1a1a;">
                  <td style="padding:10px 16px;font-size:12px;color:#666;">Balance Remaining</td>
              ...
              <p style="margin:0;font-size:12px;color:#555;line-height:1.6;">Your receipt is attached to this email as a PDF. Please keep it for your records.</p>
            </td>
          </tr>
        ...
        <table width="520" cellpadding="0" cellspacing="0" style="background:#111;border:1px solid #222;border-radius:12px;overflow:hidden;max-width:520px;width:100%;">
          ...
          <tr>
            <td style="padding:28px;">
              <p style="margin:0 0 6px;font-size:12px;color:#70c0c0;letter-spacing:1px;text-transform:uppercase;">Quotation Accepted</p>
              <h1 style="margin:0 0 20px;font-size:20px;font-weight:700;color:#fff;line-height:1.3;">${escapeHtml(quoteNumber)}</h1>
              <p style="margin:0 0 20px;font-size:14px;color:#ccc;line-height:1.6;">
                <strong style="color:#fff;">${escapeHtml(clientName)}</strong> has accepted this quotation.<br/>
          ...
          <tr>
            <td style="padding:16px 28px;border-top:1px solid #1a1a1a;">
              <p style="margin:0;font-size:11px;color:#444;">Swift Designz &middot; admin.swiftdesignz.co.za</p>
            </td>
          </tr>
        </table>
      </td>
...
export async function sendReminderEmail({
  ...
  const accentColor = isOverdue ? "#f59e0b" : "#30B0B0";
  const accentBg = isOverdue ? "#1a1000" : "#0a1a1a";
  const accentBorder = isOverdue ? "#3a2a00" : "#1a3030";

  const bodyText = stage === 1
    ? `This is a friendly reminder that payment of <strong style="color:${accentColor};">${fmtR(outstanding)}</strong> for invoice <strong style="color:#fff;">${escapeHtml(invoiceNumber)}</strong> is due in 3 days, on <strong style="color:#fff;">${dueDateFormatted}</strong>.`
    : stage === 2
    ? `This is a reminder that payment of <strong style="color:${accentColor};">${fmtR(outstanding)}</strong> for invoice <strong style="color:#fff;">${escapeHtml(invoiceNumber)}</strong> is due <strong style="color:#fff;">today, ${dueDateFormatted}</strong>.`
    : stage === 3
    ? `We note that payment of <strong style="color:${accentColor};">${fmtR(outstanding)}</strong> for invoice <strong style="color:#fff;">${escapeHtml(invoiceNumber)}</strong>, which was due on <strong style="color:#fff;">${dueDateFormatted}</strong>, is now 3 days overdue.`
    : `We urgently draw your attention to the outstanding payment of <strong style="color:${accentColor};">${fmtR(outstanding)}</strong> for invoice <strong style="color:#fff;">${escapeHtml(invoiceNumber)}</strong>, which is now 7 days overdue.`;
        ...
        <table width="520" cellpadding="0" cellspacing="0" style="background:#111;border:1px solid #222;border-radius:12px;overflow:hidden;max-width:520px;width:100%;">
          ...
          <tr>
            <td style="padding:32px;">
              ...
              <h1 style="margin:0 0 4px;font-size:22px;font-weight:700;color:#fff;">${escapeHtml(invoiceNumber)}</h1>
              <p style="margin:0 0 24px;font-size:12px;color:#555;">Due ${dueDateFormatted}</p>
              <p style="margin:0 0 24px;font-size:14px;color:#aaa;line-height:1.6;">Hi ${escapeHtml(clientName)},<br/><br/>${bodyText}</p>
              ...
              <p style="margin:0 0 24px;font-size:13px;color:#888;line-height:1.6;">Please arrange payment at your earliest convenience. If you have already made payment, please disregard this notice.<br/><br/>For any queries, please reply to this email or contact us directly.</p>
              <p style="margin:0;font-size:13px;color:#ccc;line-height:1.6;">Kind regards,<br/><strong style="color:#fff;">Swift Designz</strong></p>
            </td>
          </tr>
          <tr>
            <td style="padding:20px 32px;border-top:1px solid #1a1a1a;">
              <p style="margin:0;font-size:11px;color:#444;">Swift Designz &middot; admin.swiftdesignz.co.za &middot; keenan@swiftdesignz.co.za</p>
            </td>
          </tr>
        </table>
      </td>
